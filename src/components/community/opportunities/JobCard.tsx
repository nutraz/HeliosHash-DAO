...
import { JobPosting, JOB_CATEGORIES, CURRENCIES } from '@/types/jobs';
...
export default function JobCard({ job, onClick, className }: JobCardProps) {
  ...
  const formatCompensation = () => {
    ...
    if (job.compensation.minAmount && job.compensation.maxAmount) {
      return `${symbol}${job.compensation.minAmount.toLocaleString()} - ${symbol}${amount.toLocaleString()} /${paymentType.toLowerCase()}`;
    }
    
    return `${symbol}${amount.toLocaleString()} /${paymentType.toLowerCase()}`;
  };
