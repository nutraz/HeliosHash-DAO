...
const MOCK_JOBS: JobPosting[] = [
  {
    ...
    workType: 'FullTime',
    benefits: ['Health Insurance', 'Transportation Provided', 'Performance Bonuses', 'Training & Certification'],
    companyInfo: {
      name: 'HeliosHash DAO',
      description: 'Decentralized solar energy infrastructure organization focused on sustainable rural development'
    }
  },
  {
    id: '2',
    title: 'Community Solar Project Manager',
    description: 'Drive community engagement and coordinate solar installations across rural villages. Work directly with local leaders, manage project timelines, and ensure successful adoption of renewable energy solutions. This hybrid role combines project management with meaningful community impact.',
    category: 'Management',
    ...
    skills: ['Project Management', 'Community Relations', 'Solar Energy', 'Leadership', 'Stakeholder Management'],
  ...
  },
  {
    ...
    description: 'Join our tech team to build and maintain smart contracts for solar energy tokenization and DAO governance. Work with Internet Computer Protocol and Motoko to create transparent, efficient systems for energy trading and community governance.',
    category: 'Technology',
  ...
  },
  {
    ...
    description: 'Develop and deliver educational programs about solar energy and sustainability to local communities. Create engaging content, conduct workshops, and build digital literacy around renewable energy technologies.',
    category: 'Education',
    ...
    skills: ['Teaching', 'Content Creation', 'Public Speaking', 'Digital Literacy', 'Community Outreach'],
...
export default function JobBoard() {
  ...
  const [sortBy, setSortBy] = useState<'recent' | 'deadline' | 'featured' | 'applications'>('recent');
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      closingSoon: jobs.filter(job => job.deadline && job.deadline - now < 7 * 24 * 60 * 60 * 1000).length
    };
  }, [jobs]);
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-8">
        ...
        <div className="text-center space-y-6">
          ...
          <p className="text-xl text-gray-600 dark:text-gray-300 max-w-3xl mx-auto leading-relaxed">
            Discover solar energy jobs, contribute to DAO projects, and build a sustainable future together. 
            Join our decentralized community making renewable energy accessible to all.
          </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            { icon: Briefcase, value: stats.total, label: 'Total Jobs', color: 'blue', bgColor: 'bg-blue-50 dark:bg-blue-900/20', iconColor: 'text-blue-600', textColor: 'text-blue-600' },
            { icon: Star, value: stats.active, label: 'Active', color: 'green', bgColor: 'bg-green-50 dark:bg-green-900/20', iconColor: 'text-green-600', textColor: 'text-green-600' },
            { icon: Users, value: stats.applications, label: 'Applications', color: 'purple', bgColor: 'bg-purple-50 dark:bg-purple-900/20', iconColor: 'text-purple-600', textColor: 'text-purple-600' },
            { icon: TrendingUp, value: stats.featured, label: 'Featured', color: 'orange', bgColor: 'bg-orange-50 dark:bg-orange-900/20', iconColor: 'text-orange-600', textColor: 'text-orange-600' },
            { icon: Zap, value: stats.urgent, label: 'Urgent', color: 'red', bgColor: 'bg-red-50 dark:bg-red-900/20', iconColor: 'text-red-600', textColor: 'text-red-600' },
            { icon: Clock, value: stats.closingSoon, label: 'Closing Soon', color: 'yellow', bgColor: 'bg-yellow-50 dark:bg-yellow-900/20', iconColor: 'text-yellow-600', textColor: 'text-yellow-600' }
          ].map((stat, index) => (
            <Card 
              key={index} 
            ...
            >
              <CardContent className="p-4 text-center relative">
                ...
                <div className={`text-2xl font-bold ${stat.textColor} group-hover:scale-105 transition-transform duration-200`}>
                  {stat.value}
                </div>
                <div className="text-sm text-gray-600 dark:text-gray-400">{stat.label}</div>
                {/* Clickable indicator */}
                {(stat.label === 'Featured' || stat.label === 'Urgent' || stat.label === 'Closing Soon') && (
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <TabsList className="grid w-full md:w-auto grid-cols-2 md:grid-cols-3 bg-white dark:bg-gray-800 shadow-lg border border-gray-200 dark:border-gray-700">
              <TabsTrigger value="browse" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white font-medium">
                <Briefcase className="h-4 w-4 mr-2" />
                Browse Jobs
              </TabsTrigger>
              <TabsTrigger value="my-jobs" className="data-[state=active]:bg-green-600 data-[state=active]:text-white font-medium">
                <Users className="h-4 w-4 mr-2" />
                My Applications
              </TabsTrigger>
              {isAuthenticated && (
                <TabsTrigger value="post" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white font-medium">
          ...
          <TabsContent value="browse" className="space-y-8">
            ...
            <Card className="p-6 bg-white/80 dark:bg-gray-800/80 backdrop-blur border border-gray-200 dark:border-gray-700 shadow-lg">
              <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <Button
                    ...
                    className={`${showFilters ? "bg-blue-600 text-white shadow-lg" : "border-gray-300 hover:border-blue-500 hover:text-blue-600"} transition-all duration-200 flex-1 sm:flex-none`}
                  >
                ...
                {(searchQuery || Object.values(filters).some(v => v && (Array.isArray(v) ? v.length > 0 : true))) && (
                  <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-200 dark:border-gray-600">
                    ...
                    {filters.categories?.map(cat => (
                      <Badge key={cat} variant="secondary" className="bg-green-100 text-green-800 text-sm">
                        {cat}
                      </Badge>
                    ))}
                    {filters.experienceLevels?.map(level => (
                      <Badge key={level} variant="secondary" className="bg-purple-100 text-purple-800 text-sm">
                        {level}
                      </Badge>
                    ))}
                  </div>
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="flex-1 space-y-6">
                {filteredJobs.length > 0 ? (
                  <>
                    <div className="flex justify-between items-center bg-white/60 dark:bg-gray-800/60 backdrop-blur rounded-lg p-4 border border-gray-200 dark:border-gray-700">
          ...
          <TabsContent value="my-jobs">
            <Card className="p-12 text-center border-gray-200 dark:border-gray-700 shadow-lg bg-white/80 dark:bg-gray-800/80 backdrop-blur">
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  <h3 className="text-2xl font-semibold text-gray-700 dark:text-gray-300">My Applications & Jobs</h3>
                  <p className="text-gray-600 dark:text-gray-400 max-w-md mx-auto">
                    Track your job applications, manage posted jobs, and view application status all in one place.
                  </p>
                </div>
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <Badge variant="secondary" className="bg-yellow-100 dark:bg-yellow-900/20 text-yellow-800 dark:text-yellow-300 text-lg px-4 py-2">
                    🚧 Coming Soon
                  </Badge>
            ...
            {isAuthenticated ? (
              <Card className="p-8 bg-white/80 dark:bg-gray-800/80 backdrop-blur border border-gray-200 dark:border-gray-700 shadow-lg">
                <CardHeader className="text-center pb-8">
                  <CardTitle className="text-3xl font-bold text-gray-900 dark:text-gray-100">Post a New Job</CardTitle>
              ...
              <Card className="p-12 text-center border-gray-200 dark:border-gray-700 shadow-lg bg-white/80 dark:bg-gray-800/80 backdrop-blur">
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-2xl font-semibold text-gray-700 dark:text-gray-300">Post a Job Opportunity</h3>
                    <p className="text-gray-600 dark:text-gray-400 max-w-md mx-auto">
                      Connect with talented individuals in the renewable energy space. Please log in to post job opportunities.
                    </p>
                  </div>
                  <Button 
