...
function isNavHidden(pathname: string) {
  return ["/", "/splash", "/auth", "/onboarding", "/kyc"].some(
  ...
  return (
    <body className="bg-gradient-to-br from-navy via-dark to-darker min-h-screen flex flex-col text-light">
      <AuthProvider>
        ...
        {!hideNav && (
          <nav className="w-full bg-dark/80 backdrop-blur border-b border-gray-700 shadow-sm sticky top-0 z-40">
            <div className="max-w-4xl mx-auto flex items-center justify-between px-4 py-2">
              <ul className="flex gap-3 text-base">
                {navLinks.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-light hover:text-saffron font-medium transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
              ...
              <Image
                src="/assets/icons/hhdaologo.svg"
        ...
        <main
          id="main-content"
          className="flex-1 w-full max-w-4xl mx-auto px-4 py-4 flex flex-col items-center justify-center"
        >
          {children}
        </main>
