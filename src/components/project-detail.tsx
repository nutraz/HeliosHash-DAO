    ...
    <div className='min-h-screen app-container p-4 pb-20'>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className='w-full'>
        <TabsList className='grid w-full grid-cols-4 bg-gray-800'>
          ...
          <TabsTrigger value='milestones' className='data-[state=active]:bg-gray-700 text-white'>
            🎯 Milestones
          </TabsTrigger>
          <TabsTrigger value='production' className='data-[state=active]:bg-gray-700 text-white'>
            ⚡ Production
          </TabsTrigger>
          <TabsTrigger value='documents' className='data-[state=active]:bg-gray-700 text-white'>
            📄 Documents
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value='overview' className='space-y-3'>
          ...
          <Card className='card-readable'>
            ...
            <CardContent>
              <Progress
                value={
                  (Object.values(project.project.milestones).filter((m: any) => m.completed)
                    .length /
                    4) *
                  100
                }
              ...
              <p className='text-xs text-gray-300'>
                {Object.values(project.project.milestones).filter((m: any) => m.completed).length}{' '}
                of 4 milestones completed
              </p>
            </CardContent>
          </Card>
        </TabsContent>
            ...
            <CardContent>
                ...
                {getProductionData().map((data, index) => (
                  <div key={index} className='flex items-center justify-between'>
                    ...
                    <div className='flex items-center gap-2'>
                      ...
                      <span className='text-sm text-white font-medium'>{data.production} MW</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className='mt-4 p-3 bg-green-900/20 border border-green-600 rounded-lg'>
                ...
                <p className='text-xs text-gray-300'>
                  Average:{' '}
                  {(
                    getProductionData().reduce((sum, data) => sum + data.production, 0) / 7
                  ).toFixed(1)}{' '}
                  MW/day
                </p>
              </div>
