...
export const projectsApi = {
  async getAll(): Promise<ApiResponse<{ projects: SolarProject[] } & ProjectsStats>> {
    try {
      // Try to load projects from canister actors (tests will mock `getCanisterActors`).
      try {
        const actors = await getCanisterActors();
        const raw = (actors && actors.hhdao && typeof actors.hhdao.getProjects === 'function')
          ? await actors.hhdao.getProjects()
          : [];

        if (raw && raw.length) {
          const mapped: SolarProject[] = (raw || []).map((p: any) => ({
            id: Number(p.id),
            name: p.name || `Project ${p.id}`,
            location: p.location || '',
            capacity: Number(p.capacity ?? 0),
            status: p.status && typeof p.status === 'string' ? (p.status as any) : (p.status ? Object.keys(p.status)[0] : 'Planning'),
            owner: p.owner && typeof p.owner.toString === 'function' ? p.owner.toString() : String(p.owner || ''),
            createdAt: Number(p.createdAt || 0),
            governmentApprovals: p.governmentApprovals || [],
            telemetryId: Array.isArray(p.telemetryId) ? (p.telemetryId[0] as string | undefined) : undefined,
            description: p.description || '',
            estimatedCost: Number(p.estimatedCost ?? 0),
            completionDate: Array.isArray(p.completionDate) ? (p.completionDate[0] as number | undefined) : (typeof p.completionDate === 'number' ? p.completionDate : undefined),
          }));

          const stats: ProjectsStats = {
            totalProjects: mapped.length,
            operationalProjects: mapped.filter((pp) => pp.status === 'Operational').length,
            totalCapacity: mapped.reduce((acc, pp) => acc + (pp.capacity || 0), 0),
            totalInvestment: mapped.reduce((acc, pp) => acc + (pp.estimatedCost || 0), 0),
          };

          return { success: true, data: { projects: mapped, ...stats } };
        }
      } catch (e) {
        // ignore canister errors and fall back to demo/mock data below
      }

      // Fallback mock data used by the live demo when canisters aren't available
