...
test.describe('Dashboard Page', () => {
  ...
  test('should display women bonus badge when user is woman', async ({ page }) => {
    // Mock localStorage to simulate woman user
    await page.addInitScript(() => {
      localStorage.setItem('user', JSON.stringify({ isWoman: true, name: 'Test User' }));
    });

    await page.goto('/dashboard');

    // Check women bonus badge
  ...
  test('should display correct token balance with women bonus', async ({ page }) => {
    ...
    // Check token balance shows bonus amount
