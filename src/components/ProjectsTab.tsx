...
const ProjectsTab: React.FC<ProjectsTabProps> = ({ userRole, userId }) => {
  ...
  const fetchProjectsData = async () => {
    try {
      ...
      // Fetch applications
      const appsResponse = await fetch(`/api/projects/applications?userRole=${userRole}&userId=${userId || ''}`);
      ...
      if (userRole === 'government' || userRole === 'admin') {
        const approvalsResponse = await fetch(`/api/government/approvals?userRole=${userRole}&userId=${userId || ''}`);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-orange-50 to-red-50">
          ...
          {userRole === 'applicant' && (
            <p className="text-sm text-gray-600 mt-1">Track your solar project applications and their status</p>
          )}
        </div>
              ...
              {applications.map((application) => (
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      <div className="text-sm font-medium text-gray-900">{application.projectName}</div>
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-green-50 to-blue-50">
          ...
          <p className="text-sm text-gray-600 mt-1">Your registered land records with solar potential assessment</p>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Solar Potential</th>
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      record.verification.verified ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                    }`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button
                      ...
                      onClick={() => {/* View land record details */}}
                    >
                      View
                    </button>
                    <button
                      className="text-orange-600 hover:text-orange-900"
                      onClick={() => {/* Create application for this land */}}
                    >
                      Apply
                    </button>
                  </td>
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-purple-50">
          ...
          <p className="text-sm text-gray-600 mt-1">Track approval status from various government departments</p>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {approvals.map((approval) => (
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">{approval.departmentName}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
