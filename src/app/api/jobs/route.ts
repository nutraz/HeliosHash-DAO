...
/**
 * Fetches a filtered, sorted, and paginated list of job postings from the mock dataset using URL query parameters.
 *
 * Supported query parameters: `page`, `limit`, `category`, `location`, `search`, `featured`, `sortBy`, `sortOrder`,
 * `minCompensation`, `maxCompensation`, `experienceLevel`, `workType`, `urgency`, and `posted`.
 *
 * @param request - Incoming NextRequest whose URL search parameters control filtering, sorting, and pagination.
 * @returns A `JobListResponse` containing `jobs` (the page of results), `total` (number of matching jobs), `page`, `limit`, and `hasMore`.
 */
...
/**
 * Create a new job posting from the request JSON and add it to the mock database.
 *
 * @param request - Incoming request whose JSON body should contain the job fields to persist (e.g., title, description, category, compensation, skills, etc.)
 * @returns The created JobPosting object with an assigned `id`, `created` timestamp, `status` set to `"Active"`, empty `applicants`, and `applicationCount` 0 on success; an error object `{ error: 'Failed to create job' }` on failure.
 */
