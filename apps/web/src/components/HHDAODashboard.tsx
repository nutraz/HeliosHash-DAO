...
const HHDAODashboard = () => {
      ...
      <div className="space-y-6">
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 shadow-lg">
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setFilterStage('all')} className={`px-4 py-2 rounded-lg font-medium transition-colors ${filterStage === 'all' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
              All Projects
            </button>
            <button onClick={() => setFilterStage('functioning')} className={`px-4 py-2 rounded-lg font-medium transition-colors flex items-center space-x-2 ${filterStage === 'functioning' ? 'bg-green-600 text-white' : 'bg-green-900 bg-opacity-30 text-green-400 hover:bg-green-900 hover:bg-opacity-50 border border-green-700'}`}>
              <span className="w-3 h-3 rounded-full bg-green-500"></span>
              <span>Functioning</span>
            </button>
            <button onClick={() => setFilterStage('tech-setup')} className={`px-4 py-2 rounded-lg font-medium transition-colors flex items-center space-x-2 ${filterStage === 'tech-setup' ? 'bg-yellow-600 text-white' : 'bg-yellow-900 bg-opacity-30 text-yellow-400 hover:bg-yellow-900 hover:bg-opacity-50 border border-yellow-700'}`}>
              <span className="w-3 h-3 rounded-full bg-yellow-500"></span>
              <span>Tech Setup</span>
            </button>
            <button onClick={() => setFilterStage('solar-setup')} className={`px-4 py-2 rounded-lg font-medium transition-colors flex items-center space-x-2 ${filterStage === 'solar-setup' ? 'bg-orange-600 text-white' : 'bg-orange-900 bg-opacity-30 text-orange-400 hover:bg-orang-900 hover:bg-opacity-50 border border-orange-700'}`}>
              <span className="w-3 h-3 rounded-full bg-orange-500"></span>
              <span>Solar Setup</span>
            </button>
            <button onClick={() => setFilterStage('civil-works')} className={`px-4 py-2 rounded-lg font-medium transition-colors flex items-center space-x-2 ${filterStage === 'civil-works' ? 'bg-blue-600 text-white' : 'bg-blue-900 bg-opacity-30 text-blue-400 hover:bg-blue-900 hover:bg-opacity-50 border border-blue-700'}`}>
              <span className="w-3 h-3 rounded-full bg-blue-500"></span>
              <span>Civil Works</span>
            </button>
            <button onClick={() => setFilterStage('applied')} className={`px-4 py-2 rounded-lg font-medium transition-colors flex items-center space-x-2 ${filterStage === 'applied' ? 'bg-gray-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700 border border-gray-600'}`}>
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 shadow-lg">
          <div className="relative bg-gradient-to-br from-gray-900 to-gray-800 rounded-lg h-96 overflow-hidden border border-gray-700">
            ...
            {filteredProjects.map((project) => {
              ...
              return (
                <button key={project.id} onClick={() => setSelectedProject(project)} className={`absolute w-10 h-10 rounded-full flex items-center justify-center transform -translate-x-1/2 -translate-y-1/2 shadow-2xl hover:scale-125 transition-transform ${colorClasses.bg} ring-4 ring-gray-900`} style={{ left: `${(project.id * 15) + 20}%`, top: `${(project.id * 12) + 25}%` }}>
  ...
  const colorClasses = getColorClasses(selectedProject.color ?? 'grey');
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 shadow-lg">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <p className={`text-lg font-bold capitalize ${colorClasses.text}`}>{selectedProject.stage ? selectedProject.stage.replace('-', ' ') : ''}</p>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {selectedProject && (currentView === 'map' || currentView === 'project-detail') ? (
          <div className="mb-6">
            ...
            <button onClick={() => setSelectedProject(null)} className="text-gray-400 hover:text-gray-300 text-sm">← Back to all projects</button>
          </div>
        ) : (
          <div className="mb-6">
            <h2 className="text-2xl md:text-3xl font-bold text-white">{currentView === 'dashboard' && 'Dashboard'}{currentView === 'rewards' && 'Rewards Marketplace'}{currentView === 'map' && 'Project Map'}{currentView === 'community' && 'Community Hub'}</h2>
          </div>
        )}
