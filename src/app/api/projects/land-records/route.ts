...
export interface LandRecord {
  ...
  landClassification: 'Agricultural' | 'Residential' | 'Commercial' | 'Industrial' | 'Barren' | 'Forest';
  currentUse: string;
...
const mockLandRecords: LandRecord[] = [
  ...
  },
  {
    ...
    verification: {
      ...
      verificationNotes: "Verified for solar development project. Environmental clearance in process.",
      fieldSurveyDone: true,
      gpsVerified: true
    },
    history: {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (solarSuitable) {
      filteredRecords = filteredRecords.filter(record => 
        record.solarPotential.averageSunlight > 7 && 
        record.solarPotential.estimatedCapacity > 5
      );
    }
    ...
    const summary = {
      ...
      solarSuitableRecords: filteredRecords.filter(record => 
        record.solarPotential.estimatedCapacity > 5
      ).length,
      totalSolarPotential: filteredRecords.reduce((sum, record) => 
        sum + record.solarPotential.estimatedCapacity, 0
      ),
      byOwnershipType: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!recordData.surveyNumber || !recordData.village || !recordData.district || !recordData.ownerDetails?.name) {
      return NextResponse.json(
