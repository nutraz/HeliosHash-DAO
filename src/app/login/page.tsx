...
import { ArrowRight, Chrome, Compass, Globe, Shield, Sun, Wallet } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleWalletConnect = async (walletName: string) => {
    try {
      await login({ type: 'wallet', walletName });
      router.push('/');
    } catch (error) {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await login({ type: 'email', email, password });
      router.push('/');
    } catch (error) {
        ...
        <Card className='shadow-xl'>
          ...
          <CardContent>
            <Tabs defaultValue='internet-identity' className='w-full'>
              <TabsList className='flex w-full overflow-x-auto whitespace-nowrap md:grid md:w-auto md:grid-cols-3'>
