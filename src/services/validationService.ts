...
import { ValidationFilters, ValidationOpportunity } from '@/types/validation';
...
class ValidationService {
  ...
  async getValidationOpportunities(
    ...
    if (filters) {
      if (filters.validationType?.length) {
        opportunities = opportunities.filter((op) =>
          filters.validationType!.includes(op.validationType)
        );
      }

      if (filters.experienceLevel?.length) {
        opportunities = opportunities.filter((op) =>
          filters.experienceLevel!.includes(op.experienceLevel)
        );
      }
      ...
      if (filters.location) {
        opportunities = opportunities.filter(
          (op) =>
            op.location.address.toLowerCase().includes(filters.location!.toLowerCase()) ||
            op.projectLocation.toLowerCase().includes(filters.location!.toLowerCase())
        );
      }
      ...
      if (filters.minReward) {
        opportunities = opportunities.filter((op) => op.baseReward >= filters.minReward!);
      }

      if (filters.maxDuration) {
        opportunities = opportunities.filter((op) => op.estimatedDuration <= filters.maxDuration!);
      }
    ...
    if (userPrefersDuo) {
      opportunities.sort((a, b) => {
        ...
        const aReward = a.baseReward + (a.duoBonus || 0);
        const bReward = b.baseReward + (b.duoBonus || 0);
  ...
  async applyForValidation(
    ...
    const alreadyApplied = opportunity.currentValidators.some((v) => v.validatorId === validatorId);
    if (alreadyApplied) {
    ...
    if (
      ...
      return { success: true, message: 'Successfully applied for solo validation' };
    }

    // For duo applications
    if (sessionType === 'duo') {
      const role = opportunity.currentValidators.length === 0 ? 'Primary' : 'Secondary';
      ...
      if (opportunity.currentValidators.length === 1) {
        opportunity.status = 'PartiallyFilled';
      } else if (opportunity.currentValidators.length === opportunity.maxValidators) {
        opportunity.status = 'Full';
      }

      const message =
        role === 'Primary'
          ? 'Successfully applied for duo validation. Waiting for a partner to join.'
          : 'Successfully joined as validation partner. Duo team is now complete!';
  ...
  async getValidationStats(userPrefersDuo?: boolean): Promise<{
    ...
    const totalReward = opportunities.reduce((sum, op) => sum + op.baseReward, 0);
    const averageReward = totalReward / opportunities.length;

    const totalDuoReward = duoOpportunities.reduce(
      (sum, op) => sum + op.baseReward + (op.duoBonus || 0),
      0
    );
