...
class DisputeResolutionService {
  ...
  async createDispute(request: CreateDisputeRequest): Promise<CreateDisputeResponse> {
    try {
  ...
  ): Promise<RegisterArbitratorResponse> {
    try {
  ...
  ): Promise<ArbitratorDecisionResponse> {
    try {
  ...
  async getAllDisputes(offset: number = 0, limit: number = 50): Promise<Dispute[]> {
    try {
  ...
  async getArbitratorProfile(userId: string): Promise<ArbitratorProfile | null> {
    try {
  ...
  private categoryToVariant(category: DisputeCategory) {
    return { [category]: null };
  }

  // Helper method to convert priority to variant format
  private priorityToVariant(priority: DisputePriority) {
