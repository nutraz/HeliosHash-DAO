...
const HHDAODashboard = () => {
  ...
  // data imported from central `lib/data`
  // (use fixtures from `apps/web/src/lib/data.ts`)
  // imported at top
  // data imported from central `lib/data` (use fixtures from `apps/web/src/lib/data.ts`)

  // Project Creation Form Data
  const [projectFormStep, setProjectFormStep] = useState(1);
  ...
  const renderSettings = () => (
    <div className="space-y-4 p-6 bg-gray-900 rounded-xl">
      <h2 className="text-2xl font-bold text-white">Settings</h2>
      <p className="text-sm text-gray-400">User and application settings will appear here.</p>
    </div>
  );
    ...
    return (
      <div className="space-y-6">
        ...
        <button 
          ...
          className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl p-4 hover:shadow-2xl hover:scale-105 transition-all flex items-center justify-center space-x-2 font-bold text-lg shadow-xl"
        >
          <Plus size={24} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredProjects.filter(p => p.isLive).map((project) => {
            const colorClasses = getColorClasses(project.color);
            return (
              <button
                key={project.id}
                onClick={() => setSelectedProject(project)}
                className="bg-gradient-to-r from-cyan-900 to-purple-900 bg-opacity-50 border-2 border-cyan-500 rounded-xl p-4 hover:shadow-2xl hover:scale-105 transition-all text-left relative overflow-hidden"
              >
      ...
      <div className="space-y-6">
            ...
            <div>
              ...
              <div className="flex items-center space-x-2">
                <p className={`text-lg font-bold capitalize ${colorClasses.text}`}>
                  {selectedProject?.stage ? selectedProject.stage.replace('-', ' ') : ''}
                </p>
        ...
        {selectedProject.detailedInfo && (
          <div className="space-y-4">
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 shadow-lg">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedProject.detailedInfo.partners.map((partner, idx) => (
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 shadow-lg">
              ...
              <div className="space-y-3">
                {selectedProject.detailedInfo.milestones.map((milestone, idx) => (
            ...
            <div className="bg-gradient-to-r from-green-900 to-teal-900 bg-opacity-50 border border-green-500 rounded-xl p-6 shadow-lg">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {Object.entries(selectedProject.detailedInfo.impact).map(([key, value], idx) => (
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 shadow-lg">
          <h3 className="text-xl font-bold text-white mb-4 flex items-center space-x-2">
            ...
            <div className="space-y-3">
            {selectedProject?.opportunities?.map((opp, idx) => (
      ...
      {selectedProject ? (
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 rounded-xl p-6 text-white shadow-xl">
          ...
          <textarea
            ...
            rows={4}
            placeholder={selectedProject ? `Share updates about ${selectedProject.name}...` : "What's on your mind?"}
          />
          <div className="flex items-center justify-between mt-4">
            <div className="flex space-x-2">
              <button className="p-2 bg-gray-700 hover:bg-gray-600 rounded-lg transition-colors" aria-label="Attach image" title="Attach image">
                <Image className="text-gray-400" size={20} />
              </button>
              <button className="p-2 bg-gray-700 hover:bg-gray-600 rounded-lg transition-colors" aria-label="Attach video" title="Attach video">
                <Video className="text-gray-400" size={20} />
              </button>
              <button className="p-2 bg-gray-700 hover:bg-gray-600 rounded-lg transition-colors" aria-label="Attach file" title="Attach file">
          ...
          <div className="space-y-4 mt-6">
            <h3 className="text-lg font-bold text-white">{selectedProject ? `${selectedProject.name} - Recent Activity` : 'Recent Activity'}</h3>
            {[1, 2, 3].map((post) => (
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        ...
                        <p className="text-xs text-gray-400">2 hours ago • {selectedProject ? selectedProject.name : 'All Projects'}</p>
                      </div>
                    </div>
                    <p className="text-gray-300 mb-3">
                      Excited to share progress on our project! Small updates and community coordination here.
                    </p>
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 shadow-lg">
          <p className="text-gray-300">Select a project to view its community and activity.</p>
        </div>
      )}
    </div>
  );
  ...
  const renderProjectCreation = () => {
    ...
    const DocumentUpload = ({ label, docType, required = true, uploaded }) => (
      ...
      return (
        <div className="space-y-6">
          ...
          <div className="border-2 border-dashed border-gray-700 rounded-lg p-8 text-center bg-gray-900 bg-opacity-30">
            {!projectFormData.testimonial.videoFile ? (
              <div>
              ...
              <div className="space-y-4">
                ...
                <p className="text-sm text-gray-400">{projectFormData.testimonial.videoFile?.name}</p>
                <label
                  htmlFor="video-upload"
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center space-x-6">
              <button 
                onClick={() => { setCurrentView('dashboard'); setShowSettings(false); setSelectedProject(null); }}
                className={`px-3 py-2 rounded-lg transition-colors ${currentView === 'dashboard' && !showSettings ? 'text-blue-400 bg-blue-900 bg-opacity-30' : 'text-gray-300 hover:text-white'}`}
              >
                Dashboard
              </button>
              <button 
            ...
            <div className="flex items-center space-x-2">
              <button className="text-gray-300 hover:text-white relative" aria-label="Notifications" title="Notifications">
                <Bell size={20} />
