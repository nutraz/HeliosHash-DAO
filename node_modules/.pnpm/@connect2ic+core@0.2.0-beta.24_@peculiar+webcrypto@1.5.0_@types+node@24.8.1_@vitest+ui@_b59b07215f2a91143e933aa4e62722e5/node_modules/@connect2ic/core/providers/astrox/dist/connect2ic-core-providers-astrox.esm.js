import { _ as _classPrivateFieldGet, a as _classPrivateFieldInitSpec, b as _classPrivateFieldSet } from '../../../dist/classPrivateFieldGet-80e919e0.esm.js';
import { _ as _defineProperty } from '../../../dist/defineProperty-6a97efcd.esm.js';
import { IC } from '@astrox/sdk-web';
import { PermissionsType } from '@astrox/connection/lib/esm/types';
import { ok, err } from 'neverthrow';
import { I as InitError, C as CreateActorError, a as ConnectError, D as DisconnectError, T as TransferError, B as BalanceError } from '../../../dist/connectors-ec5b9dab.esm.js';
import { TransactionMessageKind } from '@astrox/sdk-web/build/types';

/* babel-plugin-inline-import '../assets/astrox_light.svg' */
// @ts-ignore
const astroXLogoLight = "data:image/svg+xml;base64,"; // @ts-ignore
/* babel-plugin-inline-import '../assets/astrox.png' */
const astroXLogoDark = "data:image/png;base64,";
var _config = /*#__PURE__*/new WeakMap();
var _identity = /*#__PURE__*/new WeakMap();
var _principal = /*#__PURE__*/new WeakMap();
var _ic = /*#__PURE__*/new WeakMap();
var _wallet = /*#__PURE__*/new WeakMap();
class AstroX {
  get principal() {
    return _classPrivateFieldGet(this, _principal);
  }
  get wallets() {
    return _classPrivateFieldGet(this, _wallet) ? [_classPrivateFieldGet(this, _wallet)] : [];
  }
  constructor() {
    let userConfig = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    _defineProperty(this, "meta", {
      features: ["wallet"],
      icon: {
        light: astroXLogoLight,
        dark: astroXLogoDark
      },
      id: "astrox",
      name: "AstroX ME"
    });
    _classPrivateFieldInitSpec(this, _config, {
      writable: true,
      value: void 0
    });
    _classPrivateFieldInitSpec(this, _identity, {
      writable: true,
      value: void 0
    });
    _classPrivateFieldInitSpec(this, _principal, {
      writable: true,
      value: void 0
    });
    _classPrivateFieldInitSpec(this, _ic, {
      writable: true,
      value: void 0
    });
    _classPrivateFieldInitSpec(this, _wallet, {
      writable: true,
      value: void 0
    });
    _classPrivateFieldSet(this, _config, {
      whitelist: [],
      providerUrl: "https://63k2f-nyaaa-aaaah-aakla-cai.raw.ic0.app",
      ledgerCanisterId: "ryjl3-tyaaa-aaaaa-aaaba-cai",
      ledgerHost: "https://boundary.ic0.app/",
      host: window.location.origin,
      dev: true,
      noUnify: false,
      delegationModes: ["global"],
      ...userConfig
    });
  }
  set config(config) {
    _classPrivateFieldSet(this, _config, {
      ..._classPrivateFieldGet(this, _config),
      ...config
    });
  }
  get config() {
    return _classPrivateFieldGet(this, _config);
  }
  async init() {
    try {
      const ic = await IC.create({
        useFrame: !(window.innerWidth < 768),
        signerProviderUrl: `${_classPrivateFieldGet(this, _config).providerUrl}/#signer`,
        walletProviderUrl: `${_classPrivateFieldGet(this, _config).providerUrl}/#transaction`,
        identityProvider: `${_classPrivateFieldGet(this, _config).providerUrl}/#authorize`,
        permissions: [PermissionsType.identity, PermissionsType.wallet],
        host: _classPrivateFieldGet(this, _config).host,
        ledgerCanisterId: _classPrivateFieldGet(this, _config).ledgerCanisterId,
        ledgerHost: _classPrivateFieldGet(this, _config).ledgerHost,
        dev: _classPrivateFieldGet(this, _config).dev,
        delegationTargets: _classPrivateFieldGet(this, _config).whitelist,
        delegationModes: _classPrivateFieldGet(this, _config).delegationModes,
        noUnify: _classPrivateFieldGet(this, _config).noUnify,
        customDomain: _classPrivateFieldGet(this, _config).customDomain
      });
      _classPrivateFieldSet(this, _ic, window.ic.astrox ?? ic);
      _classPrivateFieldSet(this, _principal, _classPrivateFieldGet(this, _ic).principal.toText());
      // TODO: export Identity from @astrox/connection
      // @ts-ignore
      _classPrivateFieldSet(this, _identity, _classPrivateFieldGet(this, _ic).identity);
      const isConnected = await this.isConnected();
      if (isConnected) {
        // @ts-ignore
        _classPrivateFieldSet(this, _identity, _classPrivateFieldGet(this, _ic).identity);
        _classPrivateFieldSet(this, _principal, _classPrivateFieldGet(this, _ic).principal.toText());
        _classPrivateFieldSet(this, _wallet, _classPrivateFieldGet(this, _ic).wallet);
      }
      return ok({
        isConnected
      });
    } catch (e) {
      console.error(e);
      return err({
        kind: InitError.InitFailed
      });
    }
  }
  async isConnected() {
    try {
      if (!_classPrivateFieldGet(this, _ic)) {
        return false;
      }
      return await _classPrivateFieldGet(this, _ic).isAuthenticated();
    } catch (e) {
      console.error(e);
      return false;
    }
  }

  // TODO: export & use types from astrox/connection instead of dfinity/agent
  async createActor(canisterId, idlFactory) {
    try {
      if (!_classPrivateFieldGet(this, _ic)) {
        return err({
          kind: CreateActorError.NotInitialized
        });
      }
      // @ts-ignore
      const actor = await _classPrivateFieldGet(this, _ic).createActor(idlFactory, canisterId);
      // @ts-ignore
      return ok(actor);
    } catch (e) {
      console.error(e);
      return err({
        kind: CreateActorError.CreateActorFailed
      });
    }
  }
  async connect() {
    try {
      if (!_classPrivateFieldGet(this, _ic)) {
        return err({
          kind: ConnectError.NotInitialized
        });
      }
      await _classPrivateFieldGet(this, _ic).connect({
        useFrame: !(window.innerWidth < 768),
        signerProviderUrl: `${_classPrivateFieldGet(this, _config).providerUrl}/#signer`,
        walletProviderUrl: `${_classPrivateFieldGet(this, _config).providerUrl}/#transaction`,
        identityProvider: `${_classPrivateFieldGet(this, _config).providerUrl}/#authorize`,
        permissions: [PermissionsType.identity, PermissionsType.wallet],
        host: _classPrivateFieldGet(this, _config).host,
        dev: _classPrivateFieldGet(this, _config).dev,
        ledgerCanisterId: _classPrivateFieldGet(this, _config).ledgerCanisterId,
        ledgerHost: _classPrivateFieldGet(this, _config).ledgerHost,
        delegationTargets: _classPrivateFieldGet(this, _config).whitelist,
        noUnify: _classPrivateFieldGet(this, _config).noUnify,
        delegationModes: _classPrivateFieldGet(this, _config).delegationModes,
        customDomain: _classPrivateFieldGet(this, _config).customDomain
      });
      _classPrivateFieldSet(this, _principal, _classPrivateFieldGet(this, _ic).principal.toText());
      // @ts-ignore
      _classPrivateFieldSet(this, _identity, _classPrivateFieldGet(this, _ic).identity);
      _classPrivateFieldSet(this, _wallet, _classPrivateFieldGet(this, _ic).wallet);
      return ok(true);
    } catch (e) {
      console.error(e);
      return err({
        kind: ConnectError.ConnectFailed
      });
    }
  }
  async disconnect() {
    try {
      await _classPrivateFieldGet(this, _ic)?.disconnect();
      return ok(true);
    } catch (e) {
      console.error(e);
      return err({
        kind: DisconnectError.DisconnectFailed
      });
    }
  }
  async requestTransfer(opts) {
    const {
      to,
      amount,
      standard = "ICP",
      symbol = "ICP",
      decimals = 8,
      fee = 0,
      memo = BigInt(0),
      createdAtTime = new Date(),
      fromSubAccount = 0
    } = opts;
    try {
      const result = await _classPrivateFieldGet(this, _ic)?.requestTransfer({
        amount: BigInt(amount * 10 ** decimals),
        to,
        standard,
        symbol,
        // TODO: ?
        sendOpts: {
          fee: BigInt(fee),
          memo,
          from_subaccount: fromSubAccount,
          created_at_time: createdAtTime
        }
      });
      // TODO: why string? check astrox-js
      if (typeof result === "string") {
        return err({
          kind: TransferError.FaultyAddress
        });
      }
      // check astrox-js
      if (!result) {
        return err({
          kind: TransferError.TransferFailed
        });
      }
      switch (result?.kind) {
        case "transaction-client-success":
          return ok({
            // TODO: why is payload optional? check astrox-js
            height: Number(result.payload?.blockHeight)
          });
        default:
          return err({
            kind: TransferError.TransferFailed
          });
      }
    } catch (e) {
      console.error(e);
      return err({
        kind: TransferError.TransferFailed
      });
    }
  }
  async requestTransferNFT(args) {
    try {
      const {
        tokenIdentifier,
        tokenIndex,
        canisterId,
        standard,
        to,
        fee = 0,
        memo = BigInt(0),
        createdAtTime = new Date(),
        fromSubAccount = 0
      } = args;
      if (!_classPrivateFieldGet(this, _ic)) {
        return err({
          kind: TransferError.NotInitialized
        });
      }
      const response = await _classPrivateFieldGet(this, _ic).requestTransfer({
        tokenIdentifier,
        tokenIndex,
        canisterId,
        standard,
        to,
        sendOpts: {
          fee: BigInt(fee),
          memo,
          from_subaccount: fromSubAccount,
          created_at_time: createdAtTime
        },
        symbol: ""
      });
      if (!response || typeof response === "string" || response.kind === TransactionMessageKind.fail) {
        return err({
          kind: TransferError.TransferFailed
        });
      }
      if (response.kind === TransactionMessageKind.success) {
        return ok({
          // TODO: fix
          ...response.payload
          // height: Number(response.payload.blockHeight),
        });
      }

      return err({
        kind: TransferError.TransferFailed
      });
    } catch (e) {
      console.error(e);
      return err({
        kind: TransferError.TransferFailed
      });
    }
  }
  async queryBalance() {
    try {
      const ICPBalance = Number(await _classPrivateFieldGet(this, _ic)?.queryBalance()) ?? 0;
      return ok([{
        amount: ICPBalance / 100000000,
        canisterId: _classPrivateFieldGet(this, _config).ledgerCanisterId,
        decimals: 8,
        // TODO: plug returns image?
        // image: "Dfinity.svg",
        name: "ICP",
        symbol: "ICP"
      }]);
    } catch (e) {
      console.error(e);
      return err({
        kind: BalanceError.QueryBalanceFailed
      });
    }
  }

  // async signMessage({ message }: { message: string }): Promise<SignerResponseSuccess | string | undefined> {
  //   return this.#ic?.signMessage({
  //     signerProvider: this.#config.providerUrl,
  //     message,
  //   })
  // }

  // getManagementCanister: (...args) => this.#ic.getManagementCanister(...args),
  // batchTransactions: (...args) => this.#ic.batchTransactions(...args),
}

export { AstroX };
