import { Proposal, ProposalCreateRequest } from '@/types/governance';
...
export class GovernanceService {
  static async getProposals(projectId?: string): Promise<Proposal[]> {
  ...
  static async vote(proposalId: string, support: boolean): Promise<void> {
    ...
    if (p) {
      ...
      if (p.votesFor >= 20) p.status = 'passed';
      if (p.votesAgainst >= 10) p.status = 'rejected';
    }
  }
}
