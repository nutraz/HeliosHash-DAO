...
import { Upload, File, Video, CheckCircle, Clock, AlertCircle, MapPin, Building, Plus, Edit, Eye, Download } from 'lucide-react';
  ...
  const [testimonials, setTestimonials] = useState([
    {
      ...
      content: "HHDAO transformed my 2.5 acres of unused land into a productive solar farm. Now I earn regular income while contributing to clean energy!",
    ...
    },
    {
      ...
      content: "The transparency and regular payments from HHDAO have helped our community develop infrastructure. We've built a new community center with the revenue.",
    ...
    <div className="min-h-screen app-container p-4 pb-20">
      ...
      <Tabs defaultValue="projects" className="w-full">
        <TabsList className="grid w-full grid-cols-4 bg-gray-800">
          <TabsTrigger value="projects" className="data-[state=active]:bg-gray-700 text-white">🏗️ Projects</TabsTrigger>
          <TabsTrigger value="land" className="data-[state=active]:bg-gray-700 text-white">📄 Land Records</TabsTrigger>
          <TabsTrigger value="applications" className="data-[state=active]:bg-gray-700 text-white">📋 Applications</TabsTrigger>
          <TabsTrigger value="government" className="data-[state=active]:bg-gray-700 text-white">🏛️ Government</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="projects" className="space-y-3">
          <Card className="card-readable">
            ...
            <CardContent>
                ...
                {landRecords.map((record) => (
                  <div key={record.id} className="border border-gray-600 rounded-lg p-4">
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <div className="flex gap-2 mt-1">
                          <Badge className={`${getProjectStatusColor(record.project.status)} text-white`}>
                        ...
                        {Object.entries(record.project.milestones).map(([key, milestone]) => (
                          <div key={key} className="bg-gray-800 rounded-lg p-2">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-sm">{getMilestoneIcon(key, milestone.completed)}</span>
                    ...
                    <div className="mb-3">
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-xs text-white">
                          {Object.values(record.project.milestones).filter(m => m.completed).length}/4
                        </span>
                      </div>
                      <Progress 
                        value={(Object.values(record.project.milestones).filter(m => m.completed).length / 4) * 100} 
                        className="h-2" 
                      />
                    </div>
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" className="border-gray-600 text-white flex-1" onClick={() => handleViewProject(record)}>
                        <Eye className="w-3 h-3 mr-1" />
                        View Details
                      </Button>
                      <Button size="sm" variant="outline" className="border-gray-600 text-white flex-1">
              ...
              <Button className="w-full mt-4 bg-blue-600 hover:bg-blue-700 text-white" onClick={handleNewProject}>
                  ...
                  <div key={record.id} className="border border-gray-600 rounded-lg p-3">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <h4 className="font-medium text-white">{record.owner}</h4>
                        <p className="text-sm text-gray-300">{record.location}</p>
                        <p className="text-xs text-gray-400">{record.area} • Submitted {record.submissionDate}</p>
                      </div>
              ...
              <div className="space-y-3">
                {Object.entries(governmentStatus).map(([key, status]) => (
                  <div key={key} className="border border-gray-600 rounded-lg p-3">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <h4 className="font-medium text-white capitalize">{key.replace('_', ' ')}</h4>
