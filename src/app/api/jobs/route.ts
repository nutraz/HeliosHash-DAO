...
/**
 * Fetches a filtered, sorted, and paginated list of job postings from the mock datastore.
 *
 * Supports query parameters to filter and shape results: page (number), limit (number),
 * category (comma-separated), location (comma-separated), search (text match against title/description/skills/category),
 * featured (true), sortBy (created|compensation|applications|deadline), sortOrder (asc|desc),
 ...
 *
 * @param request - Incoming NextRequest with query parameters described above.
 * @returns A JobListResponse object containing:
 *  - jobs: the page of job postings after filtering and sorting,
 *  - total: total number of jobs matching the filters,
...
/**
 * Create a new job posting from the request body and add it to the mock datastore.
 *
 * The request JSON is used as the job payload; the function augments it with an autogenerated `id`, `created` timestamp, `status` set to `"Active"`, an empty `applicants` array, and `applicationCount` set to `0` before storing.
 *
 * @param request - Incoming HTTP request whose JSON body contains the job posting fields (title, description, category, location, compensation, etc.)
 * @returns The newly created `JobPosting` object as stored in the mock datastore
 */
