...
persistent actor GovernanceRules {
    ...
    private stable var emergencyCounter : Nat = 0;
    
    // Leadership tracking
    private stable var currentLeaders : [(Principal, LeadershipRole)] = [];
    private var leadersMap = HashMap.fromIter<Principal, LeadershipRole>(
    ...
    // Emergency responses
    private stable var emergencyEntries : [(Nat, EmergencyResponse)] = [];
    private var emergencies = HashMap.fromIter<Nat, EmergencyResponse>(
    ...
    public func auditConstitutionalCompliance() : async GovernanceReport {
        ...
        let expiredEmergencies = Array.filter(
            ...
            func(e: EmergencyResponse) : Bool {
                e.status == #Active and currentTime > e.ratificationDeadline
            }
        );
        ...
        let complianceScore = (passedChecks * 100) / complianceChecks.size();
    ...
    public query func getConstitutionalConstants() : {
        votingThresholds: {
            major: Nat;
            constitutional: Nat;
            routine: Nat;
    ...
    public query func getCurrentLeaders() : [(Principal, LeadershipRole)] {
        Iter.toArray(leadersMap.entries())
    };
    
    public query func getActiveProposals() : [Proposal] {
        Array.filter(
            Iter.toArray(proposals.vals()),
            func(p: Proposal) : Bool { p.status == #Active }
        )
    ...
    public query func getActiveEmergencies() : [EmergencyResponse] {
        Array.filter(
            Iter.toArray(emergencies.vals()),
            func(e: EmergencyResponse) : Bool { 
                e.status == #Active or e.status == #PendingRatification 
            }
        )
    };
