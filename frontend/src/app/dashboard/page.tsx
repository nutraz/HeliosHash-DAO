...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    loadSolarData();
    // mark client hydration for quick debugging
    setHydrated(true);
  }, []);
  ...
  const handleViewNFTs = async () => {
    toast.info('Opening NFT Gallery... Feature: Connected to NFT membership canister');
    // TODO: Wire to NFT canister logic
  };

  const handleJoinVote = async () => {
    try {
      const result = await icpService.getProjectStats("baghpat_solar");
      console.debug('handleJoinVote result', result);
      toast.success('Active Governance Proposals loaded!');
      toast.info(`Treasury: ${result?.treasury ?? 'N/A'} | Proposals: ${result?.activeProposals ?? 'N/A'}`);
    } catch (error) {
    ...
    switch(navItem) {
      case 'projects':
        try {
          ...
          toast.info(`Active Proposals: ${stats?.activeProposals ?? 'N/A'} | Treasury: ${stats?.treasury ?? 'N/A'}`);
        } catch (error) {
      ...
      case 'rewards':
        try {
          ...
          toast.success('Rewards loaded!');
          toast.info('Available: 1,200 HHD | Pending: 450 HHD | Next reward: 2 days');
        } catch (error) {
      ...
      case 'governance':
        try {
          // TODO: Replace with real canister call
          toast.success('DAO Governance loaded!');
          toast.info('Active Proposals: 3 | Your Votes: 12 | Treasury: 1.2M HHD');
        } catch (error) {
          toast.error('Failed to load governance');
        }
        break;
      default:
        toast.info(`${navItem.charAt(0).toUpperCase() + navItem.slice(1)} Module: Connected to ${navItem} canister`);
    }
  };

  const handleNotificationClick = () => {
    setNotifications(0);
    toast.info('Real-time Notifications: New governance vote, reward available, system update complete');
  };

  const handleSettingsClick = () => {
    toast.info(`Settings: Theme: ${darkMode ? 'Dark' : 'Light'}, Network: ${process.env.NEXT_PUBLIC_IC_HOST || 'Local'}, Canisters: Connected, Version: HHDAO v1.0`);
  };
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              <img src="/assets/icons/hhdaologo.svg" alt="HeliosHash" className="w-10 h-10 object-contain" />
              <h1 className="text-xl font-bold text-slate-900 dark:text-white">HeliosHash DAO</h1>
              <div className="ml-2">
                <span className={`text-xs px-2 py-0.5 rounded-full ${hydrated ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                  {hydrated ? 'Hydrated' : 'No JS'}
                </span>
              </div>
            </div>
