...
var sdkWebview = require('@astrox/sdk-webview');
...
require('@astrox/sdk-web/build/types');
...
class ICX {
  ...
  constructor() {
    ...
    classPrivateFieldGet._classPrivateFieldSet(this, _ic, new sdkWebview.AstroXWebViewHandler());
    // // @ts-ignore
  ...
  async init() {
    try {
      await classPrivateFieldGet._classPrivateFieldGet(this, _ic).init();
      // @ts-ignore
  ...
  async isConnected() {
    try {
      ...
      const isConnected = await classPrivateFieldGet._classPrivateFieldGet(this, _ic).isConnected();
  ...
  async requestTransferNFT(args) {
    try {
      ...
      if (response.kind === types.TransactionMessageKind.fail) {
        return neverthrow.err({
...
const defaultProviders = config => {
  return isICX ? [new ICX(config)] : [new providers_astrox_dist_connect2icCoreProvidersAstrox.AstroX(config),
  // EarthWallet,
...
const walletProviders = config => {
  return isICX ? [new ICX(config)] : [new providers_astrox_dist_connect2icCoreProvidersAstrox.AstroX(config), new providers_plugWallet_dist_connect2icCoreProvidersPlugWallet.PlugWallet(config)];
};
