...
test.describe('HeliosHash smoke', () => {
  test.beforeEach(async ({ request }) => {
    ...
    if (!res.ok()) throw new Error('Server not responding on http://localhost:3002')
  })

  test('root serves HTML and contains heading', async ({ page }) => {
    const r = await page.goto('http://localhost:3002/')
  ...
  test('test-ui and test-simple pages respond', async ({ page }) => {
    const p1 = await page.goto('http://localhost:3002/test-ui/')
    expect([200, 304, 301, 302]).toContain(p1.status())
    const p2 = await page.goto('http://localhost:3002/test-simple/')
    expect([200, 304, 301, 302]).toContain(p2.status())
  })

  test('favicon and mock API health', async ({ request }) => {
    const f = await request.get('http://localhost:3002/favicon.ico')
...
test.describe('HeliosHash smoke (prod)', () => {
  test.beforeEach(async ({ request }) => {
    const res = await request.get('http://localhost:3002/')
    if (!res.ok()) throw new Error('Production server not responding on http://localhost:3002')
  })
  ...
  test('test-home and test-simple pages respond', async ({ page }) => {
    const p1 = await page.goto('http://localhost:3002/test-home/')
    expect([200, 304, 301, 302]).toContain(p1.status())
    const p2 = await page.goto('http://localhost:3002/test-simple/')
    expect([200, 304, 301, 302]).toContain(p2.status())
  })
