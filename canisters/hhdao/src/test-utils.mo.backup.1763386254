...
module TestUtils {
  ...
  public func assertResultIsOk(result : Result.Result<Any, Text>, message : Text) : () {
    switch (result) {
      case (#ok(_)) { Debug.print("✅ Assertion passed: " # message); };
      case (#err(error)) { Debug.print("❌ Assertion failed: " # message # ". Error: " # error); assert false; };
    };
  };

  public func assertResultIsError(result : Result.Result<Any, Text>, message : Text) : () {
    switch (result) {
      case (#err(err)) { Debug.print("✅ Assertion passed: " # message # ". Got expected error: " # err); };
      case (#ok(_)) { Debug.print("❌ Assertion failed: " # message # ". Expected error but got success"); assert false; };
    };
  };

  public func printTestHeader(testName : Text) : () { Debug.print("\n🧪 === Running Test: " # testName # " ==="); };
  ...
  public func mockPrincipal() : Principal { Principal.fromText("rrkah-fqaaa-aaaaa-aaaaq-cai") };
  public func mockTime() : Int { 1672531200000000000 };
