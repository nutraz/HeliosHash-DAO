...
function activate(context) {
    ...
    // Register commands
    const commands = [
        vscode.commands.registerCommand('voicenotesai.startListening', () => voiceNotesManager.startListening()),
        vscode.commands.registerCommand('voicenotesai.stopListening', () => voiceNotesManager.stopListening()),
        vscode.commands.registerCommand('voicenotesai.openPanel', () => voiceNotesManager.openPanel()),
        vscode.commands.registerCommand('voicenotesai.transcribeAudioFile', () => voiceNotesManager.transcribeAudioFile()),
        vscode.commands.registerCommand('voicenotesai.generateSummary', () => voiceNotesManager.generateSummary()),
        vscode.commands.registerCommand('voicenotesai.toggleAmbientListening', () => voiceNotesManager.toggleAmbientListening()),
    ];
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('voiceNotesPanel', provider, {
    ...
    vscode.window
        .showInformationMessage('🎤 VoiceNotes AI is ready! Use Ctrl+Shift+V to start recording.', 'Open Panel')
        .then((selection) => {
...
class VoiceNotesManager {
    ...
    async createAudioService(audioServicePath) {
...
`;
    ...
    async ensureWhisperModel() {
        ...
        const modelSize = config.get('whisperModel', 'base');
        vscode.window.showInformationMessage(`🔄 Downloading Whisper ${modelSize} model (first time only)...`);
        // Test Whisper installation
        try {
            const testProcess = (0, child_process_1.spawn)('python3', ['-c', 'import whisper; whisper.load_model("tiny")']);
            ...
            vscode.window.showErrorMessage('❌ Whisper not properly installed. Please run: pip3 install --user openai-whisper');
        }
    }
    async ensureOllamaModel() {
        const config = vscode.workspace.getConfiguration('voicenotesai');
        const modelName = config.get('ollamaModel', 'llama3.2');
        try {
            // Check if Ollama model exists
            const result = await axios_1.default.get('http://localhost:11434/api/tags');
            ...
            if (!models.some((m) => m.name.includes(modelName))) {
                vscode.window
                    ...
                    (0, child_process_1.spawn)('ollama', ['pull', modelName], { detached: true });
                });
            }
        }
        catch (error) {
    ...
    async startListening() {
        ...
        const config = vscode.workspace.getConfiguration('voicenotesai');
        const captureSystemAudio = config.get('captureSystemAudio', true);
    ...
    async stopListening() {
        ...
        try {
            ...
            if (result.text && result.text.trim()) {
                ...
                vscode.window
                    .showInformationMessage(`✅ Transcribed: "${result.text.substring(0, 50)}..."`, 'View Full Text', 'Generate Summary')
                    .then((selection) => {
    ...
    async sendAudioCommand(command) {
        return new Promise((resolve, reject) => {
            ...
            if (!this.audioService || this.audioService.killed) {
                ...
                this.audioService = (0, child_process_1.spawn)('python3', [audioServicePath]);
...
class VoiceNotesWebviewProvider {
    constructor(extensionUri, voiceNotesManager) {
