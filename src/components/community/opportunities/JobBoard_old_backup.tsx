...
import { JobFilters, JobPosting } from '@/types/jobs';
import { Briefcase, Filter, Plus, Search, Star, TrendingUp, Users } from 'lucide-react';
                ...
                {filteredJobs.length > 0 ? (
                  <div className='grid gap-4'>
                    {filteredJobs.map((job) => (
                      <JobCard key={job.id} job={job} onClick={() => handleJobSelect(job)} />
                    ))}
                  </div>
                ) : (
